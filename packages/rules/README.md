...
`packages/rules/` owns the maintained source documents, publication assets, and shared support for Gauntlet's active three-layer rules architecture.
...
- `assets/qr/` — maintained Player Guide QR assets for current digital tools.
- `rule-facts.js` — shared current-rule fact derivation and synchronization support consumed by governance validation.

## Authority boundary

Gameplay authority remains `packages/game-data/current-game.json`. These documents, assets, and helpers are maintained rules surfaces/support governed by repository contracts; they must not silently introduce mechanics that disagree with current gameplay authority.

Public URLs are independent of this repository path. `config/publication-boundary.json` materializes registered rules sources and assets to stable browser paths where required.
