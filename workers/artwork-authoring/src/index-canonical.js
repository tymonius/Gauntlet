...
async function getOverrideFile(token, cfg) {
  return github(
    `/repos/${cfg.owner}/${cfg.repo}/contents/${cfg.overridePath}?ref=${encodeURIComponent(cfg.defaultBranch)}`,
    token,
  );
}
...
async function handleSession(request, env) {
  ...
  return json({
    ...
    branch: config(env).defaultBranch,
...
async function handleArtDirection(request, env) {
  ...
  if (nextSource !== currentSource) {
    try {
      commit = await github(`/repos/${cfg.owner}/${cfg.repo}/contents/${cfg.overridePath}`, session.githubToken, {
        method: 'PUT',
      ...
      if (error.status === 403 || error.status === 409 || error.status === 422) {
        throw Object.assign(new Error(
          `GitHub blocked the canonical write to ${cfg.defaultBranch}. Check branch/ruleset permissions for the artwork-authoring GitHub App.`,
        ), { status: 409 });
      }
      throw error;
    }
  ...
  return json({
    ...
    branch: cfg.defaultBranch,
    commit: commit?.commit?.html_url ? { url: commit.commit.html_url, sha: commit.commit.sha } : null,
  }, 200, corsHeaders(request, env));
}
