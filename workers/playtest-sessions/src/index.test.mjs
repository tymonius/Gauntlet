...
test("does not expose session creation without a configured facilitator secret", async () => {
  const response = await worker.fetch(new Request("https://sessions.example/api/sessions", {
    ...
    body: JSON.stringify({ rulesVersion: "v0.6.1" })
  }), allowedEnv);
...
test("normalizes and validates v0.6.1 sheet serials", () => {
  expect(cleanSerial("g061-abcd23")).toBe("G061-ABCD23");
  expect(() => cleanSerial("v060-old-sheet")).toThrow("Invalid v0.6.1 sheet serial");
  expect(() => cleanSerial("G061-I")).toThrow("Invalid v0.6.1 sheet serial");
});
