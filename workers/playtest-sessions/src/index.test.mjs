...
test("does not expose session creation without a configured facilitator secret", async () => {
  const response = await worker.fetch(new Request("https://sessions.example/api/sessions", {
    ...
    body: JSON.stringify({ rulesVersion: "v0.6.3" })
  }), allowedEnv);
...
test("normalizes and validates v0.6.3 sheet serials", () => {
  expect(cleanSerial("g063-abcd23")).toBe("G063-ABCD23");
  expect(() => cleanSerial("G061-ABCD23")).toThrow("Invalid v0.6.3 sheet serial");
  expect(() => cleanSerial("G063-I")).toThrow("Invalid v0.6.3 sheet serial");
});
