...
async function loadRulebook() {
  ...
  try {
    ...
    status.textContent = `Clean v0.6.3 reconstruction · ${sectionCount} sections · authority ${AUTHORITY_SET_ID.slice(0, 8)}… · rendered from certified Markdown`;
  } catch (error) {
    console.error(error);
    content.removeAttribute('aria-busy');
    content.innerHTML = `
      <section class="load-error" role="alert">
        ...
        <p>Use the <a href="${SOURCE_URL}">certified clean-v0.6.3 Markdown source</a>.</p>
      </section>
    `;
