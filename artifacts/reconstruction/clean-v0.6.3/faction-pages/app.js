...
const SOURCE_URL = `../../faction-guides/${faction.authorityDir}/${faction.file}`;
sourceLinks.forEach((link) => { link.href = SOURCE_URL; });
...
async function loadGuide() {
  ...
  try {
    ...
    decorateHeadings();
    observeSections();
    status.textContent = `Certified clean v0.6.3 ${faction.label} guide loaded · SHA-256 ${actualHash.slice(0, 12)}… · authority ${AUTHORITY_SET_ID.slice(0, 12)}…`;
  } catch (error) {
    console.error(error);
    content.removeAttribute('aria-busy');
    ...
    status.textContent = `Unable to verify the certified clean v0.6.3 ${faction.label} guide.`;
  }
}
