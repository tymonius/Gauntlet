...
async function init() {
  ...
  try {
    ...
    el.dataStatus.textContent = `${state.version} clean authority · ${cardCount} playable cards + ${territoryCount} Territories loaded · publication locked`;
    el.app.hidden = false;
    render();
  } catch (error) {
    console.error(error);
    el.dataStatus.textContent = "Certified clean authority load failed";
    document.body.insertAdjacentHTML(
      "beforeend",
      `<p class="noscript">Unable to load the certified clean v0.6.3 structured authority. Serve the repository through a web server rather than opening this file directly. <a href="${CANONICAL_DATA_SOURCE}">Open the clean canonical authority</a>.</p>`
    );
  }
}
