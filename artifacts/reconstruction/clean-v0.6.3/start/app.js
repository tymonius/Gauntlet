...
const RULEBOOK_SOURCE = '../rulebook/Gauntlet_v0.6.3_Rulebook.md';
const RULEBOOK_SHA256 = '7cca20e8de2eee10332c4e3e82ca5e7abdae3a0af61837bf77caa79ccbc9d643';
const STARTERS_SOURCE = '../downstream/starter-decks.json';
...
async function fetchVerified(url, expectedSha256) {
  ...
  if (actualHash !== expectedSha256) {
    throw new Error(`Source hash mismatch for ${url}: expected ${expectedSha256}, received ${actualHash}.`);
  }
...
function extractPartOneHowItWorks(source) {
  ...
  chapterMatches.forEach((match, index) => {
    ...
    const howBody = body.slice(afterHeading, nextSecondLevel < 0 ? body.length : nextSecondLevel).trim();
    excerpts.push(`# ${title}\n\n${howBody}`);
  });
...
function buildLearningSource(source) {
  return [
    extractTopLevel(source, 'Welcome to Gauntlet'),
    extractTopLevel(source, 'Game at a Glance'),
    extractTopLevel(source, 'How to Win'),
    ...extractPartOneHowItWorks(source),
  ].join('\n\n---\n\n');
}
...
function factionLabel(id) {
  return FACTIONS.find(([factionId]) => factionId === id)?.[1] || humanizeId(id);
}

function renderFactionChoices() {
  factionChoices.replaceChildren();
  for (const [id, label, symbol] of FACTIONS) {
    const available = state.decks.some((deck) => deck.factionId === id);
    ...
    article.querySelector('input').addEventListener('change', () => {
      state.factionId = id;
      state.leaderId = '';
      renderLeaderChoices();
      renderStarter();
...
function renderStarter() {
  const deck = state.decks.find((item) => item.factionId === state.factionId && item.leaderId === state.leaderId);
  if (!deck) {
    starterPreview.innerHTML = '<p>Choose a faction and Leader to inspect the matching starter Deck.</p>';
    return;
  }
  ...
  starterPreview.innerHTML = `
    <div class="starter-heading">
      <div><p class="eyebrow">${escapeHtml(factionLabel(deck.factionId))} · ${escapeHtml(humanizeId(deck.leaderId))}</p><h3>${escapeHtml(deck.name)}</h3></div>
      <div class="starter-meta"><span>${Number(deck.cardCount)} cards</span><span>${Number(deck.deckbuildingValue)}/60 value</span></div>
    </div>
    <p class="starter-summary">${escapeHtml(deck.summary)}</p>
    <div class="starter-detail-grid">
      <div><h4>Recommended Territory order</h4><p>${territories}</p><small>Strategy recommendation from own end toward opponent end; arrangement occurs after opening selection and may be changed at setup.</small></div>
      <div><h4>Signature cards</h4><ul>${signatures}</ul></div>
    </div>
    <h4>Complete 30-card list</h4>
    <ul class="starter-card-list">${cards}</ul>`;
}

async function initialize() {
  try {
    const [rulebookSource, starterSource] = await Promise.all([
      fetchVerified(RULEBOOK_SOURCE, RULEBOOK_SHA256),
    ...
    if (!Array.isArray(starters.decks) || starters.decks.length !== 12) throw new Error('Clean Start requires exactly 12 approved starter Decks.');
    ...
    const learningSource = buildLearningSource(rulebookSource);
    ...
    renderLeaderChoices();
    renderStarter();
    sourceStatus.textContent = `Verified clean authority ${AUTHORITY_SET_ID.slice(0, 12)}… · certified Rulebook and 12 approved starter Decks loaded.`;
  } catch (error) {
    console.error(error);
    learningContent.setAttribute('aria-busy', 'false');
