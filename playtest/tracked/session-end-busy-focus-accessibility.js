(() => {
  const CONTROL_IDS = new Set(["closeSession", "cancelSession"]);

  function enhanceStatus() {
    const status = document.getElementById("sessionEndStatus");
  ...
  document.addEventListener("click", (event) => {
    ...
    const restoreObserver = new MutationObserver(() => {
      ...
      if (document.activeElement === status && target.isConnected && !target.closest("[hidden]")) {
