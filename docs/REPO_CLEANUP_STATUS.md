...
Branch: `cleanup/freeze-v062-tooling` (stacked on #1466). The remaining v0.6.2 release builders, synchronizers, renderers, and validators have moved from active `scripts/` into frozen recovery evidence with blob locks. Active lifecycle validation continues to enforce the withdrawn state, while the test boundary now checks that retired v0.6.2 tooling cannot reappear as maintained entrypoints.
