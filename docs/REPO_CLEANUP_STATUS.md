...
## Current tranche: Browser Rulebook source boundary

[#1723](https://github.com/tymonius/Gauntlet/pull/1723) separates the live Browser Rulebook application from the mixed `rulebook/` rules-source tree. The maintained browser shell, styles, rendering helpers, leader portraits, card-anatomy support, and Player's Guide review UI move under `apps/rulebook/`, while current player-facing and technical rules sources remain under `rulebook/` for the next role-classification pass.

This tranche is intentionally behavior-preserving:

- `/rulebook/` and `/rulebook/player-guide-review/` remain stable public routes;
- browser source ownership moves to `apps/rulebook/` rather than requiring a matching public-path directory;
- only explicitly declared Rulebook markdown inputs are materialized from the source-only `rulebook/` tree for browser consumption;
- the whole mixed `rulebook/` root is no longer directly published to Pages;
- publication checks, regression tests, and CI routing follow the canonical browser source location;
- historical/version-pinned release tooling and stable public-route references remain distinguishable from current source dependencies.

No gameplay authority or product behavior change belongs in this tranche. Current gameplay authority remains `packages/game-data/current-game.json`.
...
- **`rulebook/` is now a rules-source boundary, not a browser-app boundary, but remains transitional.** Player-facing authority, technical/teaching material, publication/editorial support, and version-specific rules helpers still need role classification before relocation.
...
After the Browser Rulebook/source split is green and merged:

1. Continue the `rulebook/` boundary pass: classify the remaining player-facing authority, technical/teaching rules material, publication/editorial support, and version-specific helpers before relocating any of them.
