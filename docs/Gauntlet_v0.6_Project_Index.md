...
3. **`docs/Gauntlet_v0.6_Card_Review_Log.md`** — detailed card-by-card v0.6 migration decisions, costs, wording, interactions, and rule direction.
4. **`docs/Gauntlet_Design_Principles_and_Guardrails.md`** — design principles used to evaluate proposed changes.
5. **`docs/Gauntlet_v0.6_Open_Questions.md`** — unresolved issues, contradictions, and tabled decisions.
6. **`docs/Gauntlet_v0.6_Project_Audit.md`** — audit trail comparing repo documentation against prior conversation decisions.
7. **`docs/Gauntlet_v0.6_Conversation_Audit_Leads.md`** — prior-conversation leads that still need verification or later incorporation.
8. **`docs/Gauntlet_v0.6_Leader_Design_Bible.md`** — production-facing visual direction for all twelve v0.6 faction leaders, including illustration direction, 3D miniature silhouettes, faction-pair contrast, and guardrails.
9. **`docs/Gauntlet_v0.6_Character_Design_Sheet_Log.md`** — log of generated character design sheets and useful iteration notes from the leader visual design pass.
10. **`docs/Gauntlet_v0.6_Leader_Archetype_and_Visual_Notes.md`** — leader archetype and visual direction notes; not rules text.
11. **`docs/Gauntlet_Repo_Inventory.md`** — working inventory of repo files and fetched/inspected sources.
12. **`docs/Gauntlet_v0.6_Rules_Cleanup.md`** — low-risk v0.6 cleanup items unless superseded by the working rules.
13. **`docs/Gauntlet_v0.6.1_Diplomat_Overhaul_Notes.md`** — parked Diplomat overhaul concepts for v0.6.1.
14. **`releases/v0.5.7/Gauntlet_v0.5.7_Canonical_Data.json`** — current canonical pre-v0.6 card order and card text baseline for review.
...
Before resuming in a new chat, read this index, the card metadata registry, and the detailed card review log.
...
1. Read this index.
2. Read the card metadata registry and detailed card review log.
...
5. Do not say a card decision is logged unless the metadata registry and detailed review log have both been updated where applicable.
