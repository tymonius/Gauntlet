...
export type PendingMilitaryTimingChoice =
  | {
      kind: 'brothers_in_arms_precommit';
      playerId: PlayerID;
      sourceCardId: 'military-brothers-in-arms';
      options: Array<'use' | 'pass'>;
    }
  | {
  ...
  | {
      kind: 'reserve_force_after_reveal';
      playerId: PlayerID;
      sourceCardId: 'military-reserve-force';
      storedCard?: CardID;
      handOptions: CardID[];
      options: Array<'deploy_stored' | 'replace_from_hand' | 'pass'>;
    }
  | {
  ...
  | {
      kind: 'shock_and_awe_after_reveal';
      playerId: PlayerID;
