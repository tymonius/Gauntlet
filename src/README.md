...
- `state/`, `effects/`, `cards/`, and `dev/` — pre-faction/earlier playable architecture. Useful scaffolding, but not presumed v0.7.0-compatible. Aggregate legacy APIs are explicitly versioned as `state/v06.ts`, `effects/v06.ts`, and `cards/v06.ts`. Generic state/effects barrels are removed; `cards/index.ts` is retained only as a deprecated compatibility shim while its broader legacy consumer set is migrated.\n- `cli/` and `gui/` — explicitly versioned v0.6 legacy interactive runners only; older generic v0.5.6 runners are preserved under `legacy/digital-engine-dev-runners/` as non-executable provenance.
