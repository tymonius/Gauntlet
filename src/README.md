...
- `state/`, `effects/`, `cards/`, and `dev/` — pre-faction/earlier playable architecture. Useful scaffolding, but not presumed v0.7.0-compatible. The aggregate legacy state API is explicitly versioned as `state/v06.ts`; no generic state barrel is maintained.\n- `cli/` and `gui/` — explicitly versioned v0.6 legacy interactive runners only; older generic v0.5.6 runners are preserved under `legacy/digital-engine-dev-runners/` as non-executable provenance.
