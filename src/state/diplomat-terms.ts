...
export function eligibleProposals(game: GameState, diplomatId: PlayerID): ProposalID[] {
  const player = game.players[diplomatId]; if (!player || player.factionId !== 'diplomats' || !game.battle || game.battle.stage !== 'hand_commit') return [];
...
export function openDiplomatTermsWindow(game: GameState): boolean {
  ...
  for (const playerId of [game.battle.attacker.playerId, game.battle.defender.playerId]) {
    ...
    if (eligible.length) { game.pendingDiplomatChoice = { kind: 'offer_terms', playerId, opponentId: opponentFor(game, playerId), contestedSpace: game.battle.location, eligibleProposals: eligible, options: ['offer', 'decline'] }; game.priorityPlayer = playerId; return true; }
  }
  return false;
}
export function declineTerms(game: GameState, diplomatId: PlayerID): void {
  ...
  game.pendingDiplomatChoice = undefined; game.priorityPlayer = game.battle?.attacker.playerId ?? game.activePlayer;
}
export function offerTerms(game: GameState, diplomatId: PlayerID, proposalId: ProposalID): void {
  const proposal = diplomatProposalsById.get(proposalId); const battle = game.battle;
  ...
  const opponentId = opponentFor(game, diplomatId); spendFactionResource(game, diplomatId, 'influence', proposal.stake, `Stake for ${proposal.name}`);
  ...
  game.pendingDiplomatChoice = { kind: 'respond_to_terms', playerId: opponentId, diplomatId, proposalIds: [proposalId], stake: proposal.stake, options: ['accept', 'refuse'] }; game.priorityPlayer = opponentId;
...
function captureContested(game: GameState, playerId: PlayerID): void { const b = game.battle!; const space = game.board.spaces.find(s => s.id === b.location); if (!space?.territoryId) return; const old = space.controller; if (old && old !== playerId) game.players[old].controlledTerritories = game.players[old].controlledTerritories.filter(id => id !== space.territoryId); if (!game.players[playerId].controlledTerritories.includes(space.territoryId)) game.players[playerId].controlledTerritories.push(space.territoryId); space.controller = playerId; delete space.capturePendingBy; }
function resolveAcceptedEffect(game: GameState, diplomatId: PlayerID, proposalId: ProposalID): void {
  const terms = game.players[diplomatId].diplomats!.activeTerms!; const opponent = terms.opponent;
  switch (proposalId) {
    ...
    case 'open-channels': withdraw(game, terms.defender); draw(game, opponent, 1); break;
    case 'mutual-disarmament': game.players[diplomatId].zones.discard.push(...game.players[diplomatId].zones.hand.splice(0, 1)); game.players[opponent].zones.discard.push(...game.players[opponent].zones.hand.splice(0, 1)); draw(game, opponent, 1); withdraw(game, terms.defender); break;
    case 'prisoner-exchange': { const a = game.players[diplomatId].zones.graveyard.shift(); const b = game.players[opponent].zones.graveyard.shift(); if (a) game.players[diplomatId].zones.discard.push(a); if (b) game.players[opponent].zones.discard.push(b); withdraw(game, terms.defender); break; }
    case 'rebuilding-pact': withdraw(game, terms.defender); break;
    case 'ultimatum': withdraw(game, opponent); occupyContested(game, diplomatId); break;
    case 'diplomatic-recognition': captureContested(game, diplomatId); withdraw(game, opponent); draw(game, opponent, 2); break;
  }
}
function applyRefusedEffect(game: GameState, diplomatId: PlayerID, proposalId: ProposalID): void {
  const terms = game.players[diplomatId].diplomats!.activeTerms!; const side = game.battle!.attacker.playerId === diplomatId ? game.battle!.attacker : game.battle!.defender;
  switch (proposalId) {
    ...
    case 'open-channels': side.battleDrawCount += 1; terms.refusedBattleDrawBonus = 1; break;
    case 'mutual-disarmament': if (game.players[diplomatId].zones.hand.length) game.players[diplomatId].zones.discard.push(game.players[diplomatId].zones.hand.shift()!); side.battleDrawCount += 1; terms.refusedBattleDrawBonus = 1; break;
...
export function respondToTerms(game: GameState, opponentId: PlayerID, response: 'accept' | 'refuse'): void {
  const pending = game.pendingDiplomatChoice; if (pending?.kind !== 'respond_to_terms' || pending.playerId !== opponentId) throw new DiplomatTermsError('No Terms response is pending.');
  const diplomat = game.players[pending.diplomatId]; const terms = diplomat.diplomats?.activeTerms; if (!terms) throw new DiplomatTermsError('Active Terms are missing.');
  const proposalId = terms.selectedProposalId!; terms.response = response === 'accept' ? 'accepted' : 'refused'; game.pendingDiplomatChoice = undefined;
  if (response === 'accept') {
    resolveAcceptedEffect(game, diplomat.id, proposalId); const newlyRatified = !diplomat.diplomats!.ratifiedProposals.includes(proposalId);
    gainFactionResource(game, diplomat.id, 'influence', terms.stake, 'Return accepted Stake'); if (newlyRatified) { diplomat.diplomats!.ratifiedProposals.push(proposalId); gainFactionResource(game, diplomat.id, 'influence', terms.stake, 'New Treaty Article'); }
    if (diplomat.leaderName === 'Ambassador' && diplomat.factionTriggerUsage?.ambassador_cordiality !== game.turn) { diplomat.factionTriggerUsage ??= {}; diplomat.factionTriggerUsage.ambassador_cordiality = game.turn; draw(game, diplomat.id, 1); }
    game.battle = undefined; game.phase = 'action_after_movement'; game.priorityPlayer = game.activePlayer; log(game, diplomat.id, 'terms_accepted', `${game.players[opponentId].name} accepted the Terms.`, { proposalId, newlyRatified });
  } else { applyRefusedEffect(game, diplomat.id, proposalId); game.pendingDiplomatChoice = { kind: 'leverage', playerId: diplomat.id, maximum: diplomat.resources?.influence?.value ?? 0, options: Array.from({ length: (diplomat.resources?.influence?.value ?? 0) + 1 }, (_, i) => i) }; game.priorityPlayer = diplomat.id; log(game, opponentId, 'terms_refused', `${game.players[opponentId].name} refused the Terms.`, { proposalId }); }
}
export function applyLeverage(game: GameState, diplomatId: PlayerID, amount: number): void {
  const terms = game.players[diplomatId].diplomats?.activeTerms; if (!game.battle || terms?.response !== 'refused' || !Number.isInteger(amount) || amount < 0 || amount > (game.players[diplomatId].resources?.influence?.value ?? 0)) throw new DiplomatTermsError('Leverage is not available.');
  spendFactionResource(game, diplomatId, 'influence', amount, 'Leverage'); const side = game.battle.attacker.playerId === diplomatId ? game.battle.attacker : game.battle.defender; side.modifiers += amount; terms.leverageSpent = amount; game.pendingDiplomatChoice = undefined; game.priorityPlayer = game.battle.attacker.playerId; log(game, diplomatId, 'leverage_used', `${game.players[diplomatId].name} spent ${amount} Influence on Leverage.`, { amount });
}
export function resolveRefusedTermsBattle(game: GameState, winner: PlayerID): void {
  for (const diplomat of Object.values(game.players).filter(p => p.factionId === 'diplomats')) {
    const terms = diplomat.diplomats?.activeTerms; if (terms?.response !== 'refused') continue; const proposalId = terms.selectedProposalId!;
    if (winner === diplomat.id) { gainFactionResource(game, diplomat.id, 'influence', terms.stake, 'Return imposed Stake'); if (!diplomat.diplomats!.ratifiedProposals.includes(proposalId)) diplomat.diplomats!.ratifiedProposals.push(proposalId); gainFactionResource(game, diplomat.id, 'influence', proposalId === 'ultimatum' ? 2 : proposalId === 'diplomatic-recognition' ? 0 : 1, 'Imposed Proposal'); if (terms.refusedImmediateCapture) captureContested(game, diplomat.id); }
    else { if (terms.refusedLossDraw) draw(game, diplomat.id, terms.refusedLossDraw); if (diplomat.leaderName === 'Senator' && terms.stake > 0 && diplomat.factionTriggerUsage?.senator_political_capital !== game.turn) { diplomat.factionTriggerUsage ??= {}; diplomat.factionTriggerUsage.senator_political_capital = game.turn; game.pendingDiplomatChoice = { kind: 'political_capital', playerId: diplomat.id, lostStake: terms.stake, handOptions: [...diplomat.zones.hand] }; game.priorityPlayer = diplomat.id; } }
...
export function resolvePoliticalCapital(game: GameState, playerId: PlayerID, cardIds: CardID[]): void {
  const pending = game.pendingDiplomatChoice; if (pending?.kind !== 'political_capital' || pending.playerId !== playerId || cardIds.length > pending.lostStake || cardIds.some(id => !pending.handOptions.includes(id))) throw new DiplomatTermsError('Invalid Political Capital choice.');
  const player = game.players[playerId]; for (const id of cardIds) { const i = player.zones.hand.indexOf(id); if (i >= 0) { player.zones.hand.splice(i, 1); player.zones.graveyard.push(id); gainFactionResource(game, playerId, 'influence', 1, 'Political Capital'); } }
  player.diplomats!.activeTerms = undefined; game.pendingDiplomatChoice = undefined; game.priorityPlayer = game.activePlayer;
}
