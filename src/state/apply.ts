...
function openPostBattleOrderWindow(game: GameState, winner: PlayerID): void { if (game.pendingMilitaryChoice || game.pendingMilitaryTimingChoice || game.pendingDiplomatChoice || game.players[winner].military?.victoryRestrictions?.noOrders) return; const options = legalLeaderAbilitiesFor(game, winner).filter((option) => option.timing === 'after_battle'); if (options.length) { game.pendingLeaderAbilityWindow = { playerId: winner, timing: 'after_battle', battleId: game.recentBattleResult!.battleId }; game.priorityPlayer = winner; } }
