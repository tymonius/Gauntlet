...
function recordBattleAftermath(result: ApplyGameActionResult, battle?: BattleState): void {
  ...
  correctAddedCardDestinations(result.state, battle); applyHoldTheLineLoss(result.state, battle, winner); result.state.recentBattleResult = recentBattleResult(result.state, battle, winner); applyMilitaryCommandTrigger(result.state, winner); resolveRefusedTermsBattle(result.state, winner); resolveDemilitarizedZoneBattle(result.state, battle.location); removeBlockadesAfterControlChange(result.state); recordHostileTakeoverEligibility(result.state, battle, winner); buildMilitaryAftermathChoices(result.state, battle); buildFinancierBattleAftermath(result.state, battle, winner); clearFinancierBattleState(result.state); openPostBattleOrderWindow(result.state, winner);
}
...
function finalizeLastStandResolution(result: ApplyGameActionResult, attacker?: PlayerID, defender?: PlayerID): void { if (!attacker || !defender || lastResolvedBattleWinner(result.state) !== attacker) return; result.state.winner = attacker; result.state.phase = 'game_over'; result.state.priorityPlayer = attacker; result.state.pendingMilitaryChoice = undefined; result.state.militaryChoiceQueue = undefined; result.state.pendingMilitaryTimingChoice = undefined; result.state.militaryTimingChoiceQueue = undefined; result.state.pendingDiplomatChoice = undefined; result.state.pendingFinancierChoice = undefined; result.state.financierChoiceQueue = undefined; result.state.pendingLeaderAbilityWindow = undefined; result.state.financierChoiceQueue = undefined; result.state.pendingAssetBankDiscards = undefined; appendLastStandVictoryLog(result.state, attacker, defender); }
