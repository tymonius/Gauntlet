...
describe('Operational Reassessment Battle effect', () => {
  ...
  it('removes the exact resolving copy when multiple Operational Reassessments are present', () => {
    let state = game();
    addReassessment(state, 'player_1', 'hand');
    addReassessment(state, 'player_1', 'battle_draw');
    state.players.player_1.zones.hand = ['card-valor'];
    ...
    expect(state.battle?.attacker.battleDrawPlayed).toEqual(expect.arrayContaining([
      expect.objectContaining({ cardId: 'intelligence-operational-reassessment' }),
      expect.objectContaining({ cardId: 'card-valor' }),
    ]));
  });
