...
function validateRearguardBattleAuthority(): void {
  for (const [label, cardsById] of [
    ['frozen v0.7.0', v070CanonicalContent.cardsById],
    ['current', currentCanonicalContent.cardsById],
  ] as const) {
    const card = cardsById.get(V070_REARGUARD_ID);
    const effect = card?.effects.find(
      candidate => candidate.label === 'Gambit/Tactic',
    );
    if (effect?.text !== V070_REARGUARD_BATTLE_TEXT) {
      throw new Error(
        `Rearguard battle text drifted from ${label} gameplay authority.`,
      );
    }
  }
}

validateRearguardBattleAuthority();
...
export function bankV070RearguardFromBattle(
  ...
  if (state.players[playerId].zones.assetBank.includes(rearguardInstanceId)) {
    throw new V070GameActionError('Rearguard is already banked.');
  }

  const bank = state.players[playerId].zones.assetBank;
  const atLimit = bank.length >= effectiveV070AssetLimit(state, playerId);
