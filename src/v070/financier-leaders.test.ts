...
function injectTreasuryCard(
  ...
  state.players.A.financiers!.treasury.push(instanceId);
  return instanceId;
}
...
describe('v0.7.2 Financier leaders', () => {
  ...
  test('Executive Hostile Takeover can satisfy Financial Capacity after an ordinary Opening Action', () => {
    let state = readyGame('financiers-executive-hostile-expansion');
    state.players.A.financiers!.capital = 20;
    state.players.A.position = 2;
    state.players.B.position = 3;
    ...
    const openingAction = injectHandCard(
      state,
    ...
    expect(state.players.A.financiers?.financialCapacityTurn)
      .toBe(state.turnNumber);
    expect(state.players.A.financiers?.financialCapacityQualifyingActionTurn)
      .toBeNull();
    ...
    expect(state.players.A.financiers?.financialCapacityUsedTurn)
      .toBe(state.turnNumber);
    expect(state.players.A.financiers?.financialCapacityQualifyingActionTurn)
      .toBe(state.turnNumber);
    expect(state.players.A.financiers?.financierFeatureActionSpentTurn)
      .toBeNull();
  });
