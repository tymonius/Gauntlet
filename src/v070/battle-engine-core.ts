...
function postRollRerollAvailable(
  state: V070GameState,
  effect: V070BattleCardPostRollReroll,
): boolean {
  const runtime = requireRuntime(state);
  const own = runtime.participants[effect.owner];
  const opponent = runtime.participants[otherPlayer(effect.owner)];
  if (own.battleTotal === null || opponent.battleTotal === null) return false;

  if (effect.sourceCardId === V070_VALOR_ID) {
    return own.battleTotal < opponent.battleTotal;
  }
  if (effect.sourceCardId === V070_FATES_TOLL_ID) {
    return state.players[effect.owner].zones.hand.length > 0;
  }
  return false;
}

function pruneUnavailablePostRollRerolls(
  state: V070GameState,
): void {
  const runtime = requireRuntime(state);
  const retained: V070BattleCardPostRollReroll[] = [];
  for (const effect of runtime.battleCardPostRollRerolls) {
    if (postRollRerollAvailable(state, effect)) {
...
function nextBattlePostRollPlayer(
  state: V070GameState,
  previousPlayer: PlayerId | null,
): PlayerId | null {
  const battle = requireBattle(state);
  const effects = requireRuntime(state).battleCardPostRollRerolls;
...
function openBattlePostRollChoice(
  state: V070GameState,
  playerId: PlayerId,
): boolean {
  const runtime = requireRuntime(state);
  const effects = runtime.battleCardPostRollRerolls.filter(
    effect => effect.owner === playerId,
  );
...
function resolveBattlePostRollReroll(
  ...
  if (!effect || !postRollRerollAvailable(state, effect)) {
    throw new V070GameActionError(
  ...
  } else if (costInstanceId !== undefined) {
    throw new V070GameActionError(
  ...
  runtime.battlePostRollNextPlayer =
    nextBattlePostRollPlayer(state, playerId);
  resumeBattlePostRollResolution(state);
}
...
function nextBattleAftermathControlledEffectPlayer(
  state: V070GameState,
  previousPlayer: PlayerId | null,
): PlayerId | null {
  const battle = requireBattle(state);
  const effects = remainingBattleAftermathControlledEffects(state);
...
function applyBattleAftermathControlledEffect(
  ...
  if (effect.kind === 'overlay') {
    ...
    if (index < 0) {
      throw new V070GameActionError(
        'That battle Overlay effect is no longer pending.',
      );
    }
    const [placement] =
      runtime.battleCardAftermathOverlayPlacements.splice(index, 1);
    const territory = state.board.find(
      candidate =>
        candidate.territoryInstanceId === placement.territoryInstanceId,
    );
    if (!territory) {
      appendV070Event(state, {
        type: 'battle_card_aftermath_overlay_unavailable',
        ...
        payload: {
          ...
          reason: 'contested_territory_missing',
        },
      });
      return;
    }
  ...
  const index = runtime.battleCardAftermathTerritoryInsertions.findIndex(
    insertion =>
      insertion.owner === effect.owner
      && insertion.sourceInstanceId === effect.sourceInstanceId,
  );
  if (index < 0) {
    throw new V070GameActionError(
      'That battle Territory-insertion effect is no longer pending.',
    );
  }
  const [insertion] =
    runtime.battleCardAftermathTerritoryInsertions.splice(index, 1);
...
function advanceBattleAftermathControlledEffects(
  ...
  const runtime = requireRuntime(state);
  pruneIneligibleBattleAftermathControlledEffects(state);

  if (runtime.pendingBattleAftermathControlledEffectChoice) return true;

  let nextPlayer =
    runtime.battleAftermathControlledEffectNextPlayer
    ?? nextBattleAftermathControlledEffectPlayer(state, null);

  while (nextPlayer) {
    runtime.battleAftermathControlledEffectNextPlayer = nextPlayer;
  ...
  runtime.battleAftermathControlledEffectNextPlayer = null;
  return false;
}

function resolveBattleAftermathControlledEffectChoice(
  state: V070GameState,
  playerId: PlayerId,
  sourceInstanceId: string,
  targetInstanceId?: string,
