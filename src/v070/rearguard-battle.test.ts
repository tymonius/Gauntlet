...
function readyGame(): V070GameState {
  let state = createV070StarterGame({
    gameId: 'rearguard-current-battle-test',
    seed: 'rearguard-current-battle-seed',
    players: {
      A: {
        name: 'Alpha',
...
describe('current v0.7.2 Rearguard battle effect', () => {
  test('binds the unchanged battle text to both frozen and current authority', () => {
    ...
    expect(currentCanonicalContent.cardsById.get(V070_REARGUARD_ID)?.effects
      .find(effect => effect.label === 'Gambit/Tactic')?.text)
      .toBe(V070_REARGUARD_BATTLE_TEXT);
  ...
  test('banks Rearguard after its controller loses and actually retreats', () => {
  ...
  test('at the Asset limit, Rearguard uses the controlled replacement choice', () => {
    let state = startOrdinaryAttack();
    const rearguard = inject(
      state,
      'A',
    ...
    const limit = effectiveV070AssetLimit(state, 'A');
    const existing: string[] = [];
    for (let index = 0; index < limit; index += 1) {
      existing.push(inject(
        state,
        'A',
        'neutral-foothold',
        `limit-${index}`,
