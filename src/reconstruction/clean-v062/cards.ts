...
export function activateInvasionAction(phase: ActionPhase): InvasionActionEffect {
  ...
  return { additionalAdvance: 2, advanceOnly: true, endsOnPendingBattle: true };
}

export function applyInvasionBattleMode(limits: BattleLimits, role: 'attacker' | 'defender'): BattleLimits {
  if (role !== 'attacker') return { ...limits };
  return { reserveLimit: limits.reserveLimit + 1, tacticLimit: limits.tacticLimit + 1 };
}
...
export function placeLandslide(
  ...
  if (state.overlays[territoryIndex]) throw new Error('A Territory may have no more than one Landslide.');
  return { ...state, overlays: { ...state.overlays, [territoryIndex]: owner } };
}
...
/** Resolve one-use Landslides in sequence. Consecutive Landslides can chain. */
export function resolveLandslideRetreatChain(
  ...
  while (position >= 0 && position < state.territoryCount && overlays[position]) {
    const owner = overlays[position] as PlayerId;
    delete overlays[position];
    discardedOwners.push(owner);
    position = retreatPosition(retreatingPlayer, position, state.territoryCount);
  }
  return { state: { ...state, overlays }, position, discardedOwners };
}
...
export function resolveDetenteAcceptance(
  ...
  return { state: { ...state, lastTriggeredTurn: input.turnNumber }, influenceGained: 1 };
}
...
export function bankExtraordinaryRendition(
  ...
  return {
    state: { banked: true, boundCard: { id: selectedCardId, owner: opponent, faceUp: true } },
...
export function extraordinaryRenditionDiscardOrder(assetNames: readonly string[]): string[] {
  return [
    ...assetNames.filter((name) => name === 'Extraordinary Rendition'),
    ...assetNames.filter((name) => name !== 'Extraordinary Rendition'),
  ];
}
...
export function releaseExtraordinaryRendition(
  ...
  const result = {
    ...
    discardPiles: { A: [...discardPiles.A], B: [...discardPiles.B] },
  };
...
export interface MartyrdomState {
  ...
  martyrdomAwaitingBattleCardClear: boolean;
}

export interface MartyrdomPendingResolution extends MartyrdomState {
  opponentReserveDestination: 'graveyard';
}

/**
 * Martyrdom is played before battle cards are cleared, but its Reserve
 * destination replacement and Conviction reset are completed during/after
 * battle-card clearing. Do not collapse those timings into the play response.
 */
export function playMartyrdomBeforeBattleCardsClear(
  state: MartyrdomState,
  input: { duringAftermathBeforeClear: boolean; preventedByNoMartyrs?: boolean },
): MartyrdomPendingResolution {
  ...
  if (input.preventedByNoMartyrs) throw new Error('An applicable No Martyrs prevents Martyrdom.');
  ...
  return {
    ...
    martyrdomAwaitingBattleCardClear: true,
    opponentReserveDestination: 'graveyard',
  };
}

export function clearBattleCardsWithMartyrdom(state: MartyrdomPendingResolution): MartyrdomState {
  if (!state.martyrdomAwaitingBattleCardClear) {
    throw new Error('Martyrdom has no pending battle-card-clear replacement.');
  }
  return {
    hand: [...state.hand],
