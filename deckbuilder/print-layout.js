document.addEventListener("DOMContentLoaded",()=>{window.setTimeout(installPrintLayoutOverride,0);});
...
function openPrintView(){
  ...
  if(!printWindow){window.alert("Popup blocked. Allow popups to use print/PDF export.");return;}
...
function readDeckFromPage(){
  ...
  const cardEntries=[...document.querySelectorAll("#deckCards .deck-row")].map(row=>{
    ...
    return {name,qty,cost:Number(cardRecord?.cost)||costNumber(qtyText)||"",unique:uniqueCards.has(name),action:stripLabel(texts[0]?.textContent||"","Action"),battle:stripLabel(texts[1]?.textContent||"","Battle"),reminder:stripLabel(row.querySelector(".reminder-text")?.textContent||"","Note")};
  });
  ...
  const territories=[...document.querySelectorAll("#territoryList .territory-row")]
    ...
    .map(row=>({name:row.querySelector("strong")?.textContent.trim()||"Unnamed Territory",text:row.querySelector(".card-text")?.textContent.trim()||"",arena:Boolean(row.querySelector(".arena-pill"))}));
  return {deckName,version,valid,cardCount,minimumCards,pointTotal,maximumPoints,territoryCount,territoryRequirement,cardEntries,cards,territories};
}
...
function stripLabel(text,label){return text.replace(new RegExp(`^${label}:\\s*`,"i"),"").trim();}
function costNumber(value){const match=String(value).match(/(\d+)\s*pts?/i);return match?match[1]:"";}
function chunk(items,size){const pages=[];for(let index=0;index<items.length;index+=size){pages.push(items.slice(index,index+size));}return pages;}
