(() => {
  ...
  function canonicalData() {
    if (!canonicalPromise) {
      canonicalPromise = nativeFetch(CANONICAL_URL, { cache: "no-store" }).then(async response => {
        ...
        if (data?.version !== VERSION) throw new Error(`Expected ${VERSION}, received ${data?.version || "unknown"}.`);
  ...
  loadSource = async function loadV063Source([faction, source]) {
    ...
    return (data.cards || [])
      ...
      .map(card => ({
        ...
        source: `../releases/v0.6.3/Gauntlet_v0.6.3_Complete_Card_Reference.md#${slugify(card.name)}`
      }));
  };
  ...
  renderLeader = function renderV063Leader() {
    ...
    el.leaderPreview.innerHTML = `
      ...
      <p><strong>Published v0.6.3 Leader</strong></p>
      ...
      <p class="muted">Leader abilities are governed by the published v0.6.3 rules and Faction & Component Guide.</p>
    `;
  };
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (description) description.content = "Build, validate, save, and print a canonical Gauntlet v0.6.3 Deck.";
    const eyebrow = document.querySelector(".tool-hero .eyebrow");
    if (eyebrow) eyebrow.textContent = "Playtest tool · canonical v0.6.3";
