...
async function renderProductionBack(page, baseUrl, faction) {
  ...
  const metrics = await back.evaluate((element) => {
    ...
    return {
      ...
      wordmarkMask: wordmark ? (getComputedStyle(wordmark).maskImage || getComputedStyle(wordmark).webkitMaskImage) : 'none',
  ...
  await back.screenshot({ path: join(OUTPUT_ROOT, file), omitBackground: true });
  return { faction, file };
}
...
async function renderAssets(catalog) {
  ...
  await rm(join(OUTPUT_ROOT, 'backs'), { recursive: true, force: true });
  await mkdir(join(OUTPUT_ROOT, 'cards'), { recursive: true });
  await mkdir(join(OUTPUT_ROOT, 'sheets'), { recursive: true });
  await mkdir(join(OUTPUT_ROOT, 'backs'), { recursive: true });
  ...
  try {
    ...
    for (const faction of PLAYABLE_BACK_FACTIONS) {
      const rendered = await renderProductionBack(page, baseUrl, faction);
      backVariants[faction] = rendered;
    }
...
async function main() {
  ...
  if (checkOnly) {
    console.log(`TTS v0.6.3 source check passed: ${catalog.playableCards.length} playable cards, ${catalog.territories.length} Territories, ${catalog.missingArtwork.length} cards without artwork.`);
    return;
  }
  ...
  console.log(catalogOnly
    ? `Wrote current TTS catalog to ${relative(ROOT, OUTPUT_ROOT)}.`
    : `Rendered ${catalog.playableCards.length} card images, ${PLAYABLE_BACK_FACTIONS.length} production backs, and ${Math.ceil(catalog.playableCards.length / CARDS_PER_SHEET)} TTS sheets to ${relative(ROOT, OUTPUT_ROOT)}.`);
}
