...
try {
  for (const [urlPath, filename] of sheets) {
    ...
    await page.waitForFunction(() => window.__cardsReady === true || Array.isArray(window.__cardFitResults), null, { timeout: 30000 });
    await page.evaluate(async () => { if (document.fonts?.ready) await document.fonts.ready; });
    const results = await page.evaluate(() => window.__cardFitResults ?? []);
