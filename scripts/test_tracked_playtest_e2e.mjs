...
try {
  ...
  assert.equal(health.version, "v0.7.2");
  ...
  assert.equal(created.rulesVersion, "v0.7.2");
  assert.match(created.sheetSerial, /^G072-[A-Z0-9]{8}$/);
  ...
  assert.equal(initial.rulesVersion, "v0.7.2");
  ...
  const linked = await json(await call(`/api/tracked-games/${created.joinToken}/arbiter`, {
    ...
    body: auth(playerTwo, {
      ...
      sources: [{ title: "v0.7.2 Rulebook", section: "Capture" }]
    })
  }), 201);
  ...
  console.log("Validated v0.7.2 public tracked creation -> two player seats -> authenticated milestones -> player-attributed Arbiter linkage -> shared result -> two private responses -> automatic closure -> private review/export data.");
} finally {
  db.close();
}
