...
requireValue(data.cards.length === 129, `Expected 129 playable cards, found ${data.cards.length}.`);
...
for (const [pool, count] of Object.entries(expectedPools)) {
  requireValue(data.card_pool_summary[pool]?.count === count, `${pool} pool should contain ${count} titles.`);
}

for (const entry of [...data.cards, ...data.territories]) {
  requireValue(!Object.hasOwn(entry, "complexity"), `${entry.name}: retired complexity field remains active.`);
}
...
for (const retired of ["Action Opportunity", "opening effects", "battle opening", "revealed Territory", "Defender's Advantage"]) {
  forbidText(serialized, retired, "effective canonical data");
}
...
console.log(`v0.6.2 Wave D validation passed: ${data.cards.length} cards, ${data.territories.length} Territories, ${data.proposals.length} Proposals, ${starters.decks.length} starters, and three candidate player surfaces.`);
