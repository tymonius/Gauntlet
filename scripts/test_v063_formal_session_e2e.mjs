...
try {
  ...
  assert.match(rejectedLegacyCreation.error, /v0\.7\.2 sessions only/i);
  ...
  const event = await json(await call("/api/sessions", {
    ...
    body: {
      rulesVersion: "v0.7.2",
      sessionKind: "event",
      metadata: { label: "Automated v0.7.2 game night" }
    }
  }), 201);
  assert.equal(event.rulesVersion, "v0.7.2");
  assert.equal(event.sessionKind, "event");
  assert.match(event.sheetSerial, /^EV072-[A-Z0-9]{8}$/);
  ...
  await db.prepare(`INSERT INTO rules_interactions
    ...
    VALUES (?, ?, 1, ?, ?, ?, ?, 'v0.7.2', 'explicit', 'high', 'retrieval_only', 1)`)
    .bind(
      interactionId,
      "arbiter-v072-event-game-e2e",
  ...
  const linked = await json(await call(`/api/sessions/${gameOne.joinToken}/arbiter`, {
    ...
    body: {
      ...
      sources: [{ title: "v0.7.2 Rulebook", section: "Aftermath" }]
    }
  }), 201);
  ...
  assert.equal(gameAfterActivity.rulesVersion, "v0.7.2");
  ...
  const standalone = await json(await call("/api/sessions", {
    ...
    body: {
      rulesVersion: "v0.7.2",
      sheetSerial: "G072-STD001",
      metadata: { formal: true }
    }
  }), 201);
  assert.equal(standalone.rulesVersion, "v0.7.2");
  assert.equal(standalone.sheetSerial, "G072-STD001");
  ...
  console.log("Validated v0.7.2 event and standalone creation, G072/EV072 serials, table-game onboarding and Arbiter linkage, closure, rejection of older-version creation, and read compatibility for stored v0.6.1/v0.7.0 sessions.");
} finally {
  db.close();
}
