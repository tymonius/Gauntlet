...
try {
  ...
  assert.match(rejectedLegacyCreation.error, /v0\.6\.3 sessions only/i);
  ...
  const event = await json(await call("/api/sessions", {
    ...
    body: {
      rulesVersion: "v0.6.3",
      sessionKind: "event",
      metadata: { label: "Automated v0.6.3 game night" }
    }
  }), 201);
  assert.equal(event.rulesVersion, "v0.6.3");
  assert.equal(event.sessionKind, "event");
  assert.match(event.sheetSerial, /^EV063-[A-Z0-9]{8}$/);
  ...
  const linked = await json(await call(`/api/sessions/${gameOne.joinToken}/arbiter`, {
    ...
    body: {
      ...
      sources: [{ title: "v0.6.3 Rulebook", section: "Aftermath" }]
    }
  }), 201);
  ...
  assert.equal(gameAfterActivity.rulesVersion, "v0.6.3");
  ...
  const standalone = await json(await call("/api/sessions", {
    ...
    body: {
      rulesVersion: "v0.6.3",
      sheetSerial: "G063-STD001",
      metadata: { formal: true }
    }
  }), 201);
  assert.equal(standalone.rulesVersion, "v0.6.3");
  assert.equal(standalone.sheetSerial, "G063-STD001");
  ...
  console.log("Validated v0.6.3 event and standalone creation, G063/EV063 serials, table-game onboarding and Arbiter linkage, closure, rejection of new v0.6.1 creation, and read compatibility for stored v0.6.1 sessions.");
} finally {
  db.close();
}
