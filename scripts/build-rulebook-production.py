#!/usr/bin/env python3
"""Adapt the approved Rulebook production system to a supplied release source."""
...
PUBLICATION_ROOT = ROOT / "legacy" / "v0.6.1-rulebook-publication"
PRODUCTION = PUBLICATION_ROOT / "rulebook-production"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source", type=Path, required=True, help="Rulebook Markdown source, relative to the repository root or absolute.")
    parser.add_argument("--version", required=True, help="Target release identity, such as v0.7.1.")
    parser.add_argument(
        "--production-source",
        type=Path,
        default=Path("legacy/v0.6.1-rulebook-publication/rulebook-production/.rulebook-production-source.md"),
        help="Generated presentation Markdown, relative to the repository root or absolute.",
    )
    parser.add_argument(
        "--validate-player-chapter-11",
        action="store_true",
        help="Require the source and presentation copy to contain the reviewed player-facing Chapter 11.",
    )
...
def main() -> None:
    ...
    production_source_path = resolve_repo_path(args.production_source)
    target_version = args.version.removeprefix("v")
    ...
    if args.validate_player_chapter_11:
        assert_reviewed_chapter_11(current_source, "Player-facing Rulebook input")
    ...
    production_source_path.write_text(production_source, encoding="utf-8")
    build_rulebook.RULEBOOK = production_source_path
    build_complete_rulebook.main()

    html = HTML.read_text(encoding="utf-8")
    html = replace_required(html, "Version 0.6.1 · First Playtest Revision", f"Version {target_version}", "approved cover version")
    html = replace_required(html, "Gauntlet v0.6.1 Official Rulebook", f"Gauntlet v{target_version} Official Rulebook", "document title", 2)
    html = html.replace("GAUNTLET V0.6.1", f"GAUNTLET V{target_version.upper()}")
    html = html.replace("Gauntlet v0.6.1 · First Playtest Revision", f"Gauntlet v{target_version}")
    html = html.replace("Gauntlet v0.6.1", f"Gauntlet v{target_version}")
    html = html.replace("../images/", "../../../images/")
    HTML.write_text(html, encoding="utf-8")

    paginator = RUNTIME_PAGINATOR.read_text(encoding="utf-8")
    paginator = replace_required(paginator, "GAUNTLET V0.6.1", f"GAUNTLET V{target_version.upper()}", "folio version", 3)
    paginator = paginator.replace("Gauntlet v0.6.1", f"Gauntlet v{target_version}")
    ...
    paginator = paginator.replace("../images/", "../../../images/")
    RUNTIME_PAGINATOR.write_text(paginator, encoding="utf-8")
    print(f"adapted approved Rulebook production to v{target_version} from {source_path.relative_to(ROOT)} with wording-neutral presentation transforms, minimum-count hierarchical filler planning, and unique hero filler art")
