...
const urls = {
  ...
  arcaneTrait: 'https://gauntlet.run/releases/v0.7.0/Gauntlet_v0.7.0_Arcane_Trait_Mark.png',
...
for (const key of ['website', 'rulebook', 'browser_tools', 'rules_arbiter', 'digital_rules']) {
  if (manifest.public_defaults?.[key] !== 'v0.7.0') throw new Error(`Public manifest default ${key} is not v0.7.0.`);
}
if (
  ...
  throw new Error('Public v0.7.0 provenance does not match the manifest authority.');
}
if (
  ...
  throw new Error('Public v0.7.0 canonical data does not match the promoted current-game authority.');
}
...
if (!bookletType.includes('pdf')) {
  throw new Error(`Public v0.7.0 Rulebook booklet did not resolve as PDF: ${bookletType}`);
}
...
console.log('gauntlet.run live verification passed for v0.7.0 publication with bound Rulebook, canonical data, provenance, print-adapted Card Anatomy figures, booklet, and Rules Arbiter assets.');
