...
const manifest = {
  ...
  note: 'The aggregate closeout manifest fingerprints tracked source, print-semantic, browser, Rules Arbiter, digital, and faction-symbol surfaces. Raw rendered PDF bytes are excluded because renderer metadata is nondeterministic; the workflow re-renders and validates every PDF from the fingerprinted print semantics on each closeout run.',
};
