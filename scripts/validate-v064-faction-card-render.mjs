...
const OUTPUT = join(ROOT, 'card-design', 'generated', 'v064-faction-candidates');
const SOURCE_PATH = join(ROOT, 'docs', 'v0.6.4-faction-card-additions.json');
const CARD_WIDTH = 240;
const CARD_HEIGHT = 336;
const EXPECTED_SOURCE_ISSUE = 576;
const EXPECTED_CANDIDATE_COUNT = 6;
const EXPECTED_CATALOG_COUNT = 134;
...
function validateSource(source) {
  if (source.version !== 'v0.6.4-candidate' || source.base_version !== 'v0.6.3') {
    throw new Error('Faction-card render validation requires the v0.6.4 candidate overlay on v0.6.3.');
  }
  if (source.source_issue !== EXPECTED_SOURCE_ISSUE || source.ready_for_game_data !== false) {
    throw new Error('Faction-card candidate source must remain pinned to issue #576 and outside canonical game data.');
  }
  if (!Array.isArray(source.cards) || source.cards.length !== EXPECTED_CANDIDATE_COUNT) {
    throw new Error(`Expected ${EXPECTED_CANDIDATE_COUNT} faction-card candidates.`);
  }
}
...
async function main() {
  ...
  try {
    ...
    await catalogPage.waitForFunction(() => document.body.dataset.v064FactionCards === 'ready');
    ...
    for (const sourceCard of source.cards) {
      ...
      const metric = await page.locator('.gauntlet-card').evaluate(card => {
        ...
        const labels = [...card.querySelectorAll('.rule-section h4')].map(node => node.textContent?.trim());
        return {
      ...
      if (metric.gameVersion !== 'v0.6.4 candidate'
        || metric.sourceHierarchy?.[0] !== '/docs/v0.6.4-faction-card-additions.json') {
    ...
    await writeFile(join(OUTPUT, 'metrics.json'), `${JSON.stringify({
      sourceIssue: EXPECTED_SOURCE_ISSUE,
