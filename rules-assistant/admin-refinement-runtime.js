...
export function adminRefinementRuntimeSource() {
  ...
  function reportStatus(report){if(report.scope==='reviewed_backlog')return report.stats.eligible?'Found '+report.stats.eligible+' reviewed interaction'+(report.stats.eligible===1?'':'s')+' with refinement signals. '+report.stats.high+' are high priority across '+report.stats.clusters+' root-cause cluster'+(report.stats.clusters===1?'':'s')+'.':'No reviewed interactions currently carry deterministic refinement signals.';return report.stats.unreviewed?'Scored '+report.stats.unreviewed+' unreviewed interaction'+(report.stats.unreviewed===1?'':'s')+'. '+report.stats.attention+' currently need attention.':'All recorded interactions are already reviewed; there is nothing new to triage.'}
  function refresh(){
