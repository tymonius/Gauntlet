...
export function adminRefinementRuntimeSource() {
  ...
  function reportStatus(report){var resolved=Number(report.stats.resolvedByRefinement||0),suffix=resolved?' '+resolved+' previously flagged interaction'+(resolved===1?' is':'s are')+' retired by the refinement ledger.':'';if(report.scope==='reviewed_backlog')return report.stats.eligible?'Found '+report.stats.eligible+' unresolved reviewed interaction'+(report.stats.eligible===1?'':'s')+' with refinement signals. '+report.stats.high+(report.stats.high===1?' is':' are')+' high priority across '+report.stats.clusters+' root-cause cluster'+(report.stats.clusters===1?'':'s')+'.'+suffix:'No unresolved reviewed interactions currently carry deterministic refinement signals.'+suffix;return report.stats.unreviewed?'Scored '+report.stats.unreviewed+' unreviewed interaction'+(report.stats.unreviewed===1?'':'s')+'. '+report.stats.attention+' currently need attention.':'All recorded interactions are already reviewed; there is nothing new to triage.'}
  function refresh(){
