...
const ACCEPTED_VERSION_ALIASES = new Set([RULES_VERSION, "v0.6.2"]);
...
const SYSTEM_PROMPT = `You are the Gauntlet Rules Arbiter for the v0.6.2 candidate playtest rules.

Use only the supplied v0.6.2 candidate sources, recent conversation, prior session rulings, and the adjudication principles below. Do not import v0.6.1 mechanics except when the question explicitly asks for a comparison.
...
export default {
  async fetch(request, env = {}) {
    ...
    if (request.method === "GET" && ["/api/v062/health", "/v062/health"].includes(url.pathname)) {
      return jsonResponse({
        ok: true,
    ...
    if (request.method !== "POST" || !["/api/v062/rules", "/v062/rules"].includes(url.pathname)) {
    ...
    if (!ACCEPTED_VERSION_ALIASES.has(requestedVersion)) {
      return jsonResponse({
        error: `This candidate Rules Arbiter answers ${RULES_VERSION} questions only.`
      }, 409, allowedOrigin);
    }
    ...
    try {
      ...
      if (corpus.version !== RULES_VERSION) {
        throw new Error(`Candidate corpus reports ${String(corpus.version)}, expected ${RULES_VERSION}.`);
      }
      ...
      try {
        ...
        console.error("v0.6.2 candidate model call failed; using source lookup", error);
        const fallback = buildLocalFallbackAnswer(question, retrieval, corpus.version);
        return finishAnswer({
          request,
          env,
      ...
      console.error("v0.6.2 candidate Rules Arbiter failure", error);
      return jsonResponse({
        error: "The v0.6.2 candidate Rules Arbiter could not complete the request."
      }, 502, allowedOrigin);
    }
  }
};
...
async function getCorpus(env) {
  if (!corpusPromise) {
    corpusPromise = loadV062RulesCorpus(defaultV062SourceUrls(env.SITE_ORIGIN || "https://gauntlet.run")).catch((error) => {
      corpusPromise = null;
      throw error;
    });
  }
