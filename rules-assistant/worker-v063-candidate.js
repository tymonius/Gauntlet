...
export const RULES_VERSION = V063_RULES_VERSION;
const VERSION_ALIASES = new Set([RULES_VERSION, "v0.6.3-candidate", "v0.6.3"]);
...
const SYSTEM_PROMPT = `You are the Gauntlet Rules Arbiter for the unpublished v0.6.3 development candidate.

Use only the supplied v0.6.3 candidate sources and conversation context. v0.6.2 remains the published playtest edition, but where the candidate expressly changes a rule or card, the v0.6.3 candidate controls this answer. Do not invent a release status or import obsolete v0.6.1/v0.6.2 text that the candidate has replaced.

Classify every gameplay answer as exactly one of:
- explicit: candidate text directly states the answer;
- inferred: the answer is compelled by applying candidate rules with no discretionary gap;
- provisional: the candidate leaves a genuine gap or ambiguity;
...
10. A provisional ruling must begin with "Provisional Arbiter Ruling:" and clearly say it is a development ruling rather than published v0.6.2 law.
...
export default {
  async fetch(request, env = {}) {
    ...
    if (request.method === "GET" && ["/health", "/api/health", "/v063/health", "/api/v063/health"].includes(url.pathname)) {
      return json({
        ok: true,
        service: "gauntlet-rules-assistant-v063-candidate",
        version: RULES_VERSION,
        versionLabel: V063_VERSION_LABEL,
        candidate: true,
    ...
    if (!VERSION_ALIASES.has(requestedVersion)) {
      return json({ error: `This development Rules Arbiter answers ${V063_VERSION_LABEL} questions only.` }, 409, origin);
    }
    ...
    try {
      ...
      try {
        ...
        console.error("v0.6.3 candidate model call failed; using source lookup", error);
        const fallback = buildLocalFallbackAnswer(question, retrieval, RULES_VERSION);
        return answerResponse({
          answer: fallback.answer,
          rulingStatus: fallback.rulingStatus,
      ...
      if (rulingStatus === "provisional" && !/^Provisional Arbiter Ruling:/i.test(answer)) {
        answer = `Provisional Arbiter Ruling: ${answer} This is a v0.6.3 development ruling and does not alter the published v0.6.2 rules.`;
      }
      return answerResponse({
        answer,
        rulingStatus,
      ...
      console.error("v0.6.3 candidate Rules Arbiter failure", error);
      return json({ error: "The v0.6.3 candidate Rules Arbiter could not complete the request." }, 502, origin);
    }
  }
};
...
async function getCorpus(env) {
  if (!corpusPromise) {
    corpusPromise = loadDevelopmentV063RulesCorpus({
      ...defaultDevelopmentV063SourceUrls(env.SITE_ORIGIN || "https://gauntlet.run"),
      fetchImpl: fetch
