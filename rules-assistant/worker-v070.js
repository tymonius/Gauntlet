...
import {
  ...
  loadV070RulesCorpus
} from "./v070-public-corpus.js";

export const RULES_VERSION = V070_RULES_VERSION;
...
const SYSTEM_PROMPT = `You are the Gauntlet Rules Arbiter for the current canonical v0.7.0 playtest edition.

Use only the supplied published v0.7.0 release passages, recent conversation, prior session rulings, and adjudication principles supplied with the question. Do not use outside knowledge, later development material, withdrawn Gauntlet releases, historical candidate text, or unstated design facts.
...
export default {
  async fetch(request, env = {}) {
    ...
    if (
      request.method === "GET" &&
      ["/", "/health", "/api/health", "/v070/health", "/api/v070/health"].includes(url.pathname)
    ) {
      return json({
        ok: true,
    ...
    if (
      request.method !== "POST" ||
      !["/rules", "/api/rules", "/v070/rules", "/api/v070/rules"].includes(url.pathname)
    ) {
    ...
    if (requestedVersion !== RULES_VERSION) {
      return json({
        error: `This Rules Arbiter answers ${V070_VERSION_LABEL} questions only.`
      }, 409, origin);
    }
    ...
    try {
      ...
      console.error("v0.7.0 Rules Arbiter failure", error);
...
async function getCorpus(env) {
  if (!corpusPromise) {
    const urls = defaultV070SourceUrls(env.SITE_ORIGIN || "https://gauntlet.run");
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        ...
        format: {
          type: "json_schema",
          name: "gauntlet_v070_rules_answer",
          strict: true,
...
async function makeSafetyIdentifier(request, env) {
  const salt = env.SAFETY_ID_SALT || "gauntlet-v070-rules-arbiter";
