...
const SYSTEM_PROMPT = `You are the Gauntlet Rules Arbiter for the current canonical v0.6.3 playtest edition.

Use only the supplied certified v0.6.3 source passages, recent conversation, prior session rulings, and adjudication principles supplied with the question. Do not use outside knowledge, withdrawn Gauntlet releases, historical candidate text, or unstated design facts.
...
export default {
  async fetch(request, env = {}) {
    ...
    if (
      request.method === "GET" &&
      ["/", "/health", "/api/health", "/v063/health", "/api/v063/health"].includes(url.pathname)
    ) {
      return json({
        ok: true,
    ...
    if (
      request.method !== "POST" ||
      !["/rules", "/api/rules", "/v063/rules", "/api/v063/rules"].includes(url.pathname)
    ) {
    ...
    if (requestedVersion !== RULES_VERSION) {
      return json({
        error: `This Rules Arbiter answers ${CLEAN_V063_VERSION_LABEL} questions only.`
      }, 409, origin);
    }
...
async function makeSafetyIdentifier(request, env) {
  const salt = env.SAFETY_ID_SALT || "gauntlet-v063-rules-arbiter";
