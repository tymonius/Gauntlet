...
export const V070_PROVENANCE_SOURCE_PATH = 'releases/v0.7.0/Gauntlet_v0.7.0_Source_Provenance.json';
...
export function defaultV070SourceUrls(origin = 'https://gauntlet.run') {
  ...
  return {
    siteOrigin: base,
    rulebookUrl: `${base}/${V070_RULEBOOK_SOURCE_PATH}`,
    canonicalDataUrl: `${base}/${V070_CANONICAL_SOURCE_PATH}`,
    manifestUrl: `${base}/${V070_MANIFEST_SOURCE_PATH}`,
    provenanceUrl: `${base}/${V070_PROVENANCE_SOURCE_PATH}`,
    rulebookBrowserUrl: `${base}/rulebook/`,
    rulebookPdfUrl: `${base}/releases/v0.7.0/Gauntlet_v0.7.0_Rulebook_Booklet.pdf`,
  };
}
...
function requireBinding(manifest, key, digest) {
  ...
  if (!binding?.sha256) throw new Error(`v0.7.0 manifest is missing ${key} binding.`);
  if (binding.sha256 !== digest) {
    throw new Error(`v0.7.0 ${key} hash mismatch: expected ${binding.sha256}, received ${digest}.`);
  }
}

export function validateV070PublishedData({ canonicalData, manifest, provenance, rulebookMarkdown } = {}) {
  if (!canonicalData || canonicalData.release_version !== V070_RULES_VERSION) {
    throw new Error('Published v0.7.0 canonical data has the wrong release identity.');
  }
  if (canonicalData.source_version !== V070_SOURCE_VERSION) {
    throw new Error('Published v0.7.0 canonical data has the wrong source identity.');
  }
  if (
    ...
    throw new Error('Published v0.7.0 canonical data has incomplete gameplay counts.');
  }
  ...
  if (!canonicalData.leaders.every(leader =>
    ...
    throw new Error('Published v0.7.0 Leader data is not structurally current.');
  }
  ...
  if (retired.test(String(rulebookMarkdown || ''))) {
    throw new Error('Published v0.7.0 Rulebook contains retired current terminology.');
  }
  if (retired.test(JSON.stringify(canonicalData))) {
    throw new Error('Published v0.7.0 canonical data contains retired current terminology.');
  }

  if (
    manifest?.release_version !== V070_RULES_VERSION
    || manifest?.status !== 'current'
    || !manifest?.authority_set_id
  ) {
    throw new Error('Published v0.7.0 manifest is incomplete.');
  }
  if (
    provenance?.release_version !== V070_RULES_VERSION
    || provenance?.source_version !== V070_SOURCE_VERSION
    || provenance?.authority_set_id !== manifest.authority_set_id
  ) {
    throw new Error('Published v0.7.0 provenance is not bound to the release manifest.');
  }
  return true;
}

export async function loadV070RulesCorpus(options = {}) {
  const defaults = defaultV070SourceUrls(options.siteOrigin || globalThis.location?.origin || 'https://gauntlet.run');
  const urls = { ...defaults, ...options };
  const fetchImpl = options.fetchImpl || globalThis.fetch;
  if (typeof fetchImpl !== 'function') throw new Error('A fetch implementation is required to load v0.7.0 rules.');
  ...
  for (const [label, response] of [
    ...
    if (!response.ok) throw new Error(`Published v0.7.0 ${label} returned ${response.status}.`);
  }
  ...
  validateV070PublishedData({ canonicalData, manifest, provenance, rulebookMarkdown });
  ...
  const documents = corpus.documents.map(document => document.kind === 'rulebook'
    ? {
        ...document,
        sourcePath: V070_RULEBOOK_SOURCE_PATH,
        sourceUrl: document.sourceUrl || urls.rulebookBrowserUrl,
      }
    : {
        ...document,
        sourcePath: V070_CANONICAL_SOURCE_PATH,
