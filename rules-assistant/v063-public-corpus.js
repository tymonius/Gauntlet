...
export async function loadV063RulesCorpus({
  rulebookUrl,
  canonicalDataUrl,
  rulebookBrowserUrl,
  ...
  const defaults = defaultV063SourceUrls(
  ...
  if (!rulebookResponse.ok) {
    throw new Error(`v0.6.3 Rulebook returned ${rulebookResponse.status}.`);
  }
  if (!canonicalResponse.ok) {
    throw new Error(`v0.6.3 canonical data returned ${canonicalResponse.status}.`);
  }

  const [rulebookBytes, canonicalBytes] = await Promise.all([rulebookResponse.arrayBuffer(), canonicalResponse.arrayBuffer()]);
  ...
  corpus.reconstruction = false;
  corpus.published = true;
  ...
  corpus.documents = corpus.documents.map((document) => {
    if (document.kind === "rulebook") {
      return {
        ...document,
        sourcePath: V063_PUBLISHED_RULEBOOK_PATH,
...
export function validateV063Inputs({ rulebookMarkdown, canonicalData } = {}) {
  const rulebook = String(rulebookMarkdown || "");
  if (!rulebook.includes("# GAUNTLET") || !rulebook.includes("## Official Rulebook")) {
    throw new Error("v0.6.3 Rulebook is missing the expected GAUNTLET / Official Rulebook headings.");
  }

  if (!canonicalData || typeof canonicalData !== "object") {
    throw new Error("v0.6.3 canonical data is missing.");
  }
