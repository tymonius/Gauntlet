...
const QUERY_PHRASE_ALIASES = [
  ...
  {
    pattern: /(?=[\s\S]*\b(?:apply|applies|applied|applying|repeat|repeats|repeated|repeating)\b)(?=[\s\S]*\b(?:opponent(?:['’]s)|another\s+(?:player|card)(?:['’]s)?|other\s+player(?:['’]s)?|source\s+card|as\s+though|as\s+if)\b)/i,
...
function scoreDocument(document, normalizedQuery, baseQueryTokens, queryTokens, queryPhrases, conversationFocus) {
  ...
  // Only boost single-word titles introduced by alias expansion; boosting literal query tokens destabilizes established retrieval ordering.\n  if (rawTitle.length > 4 && !rawTitle.includes(" ") && queryTokens.includes(rawTitle) && !baseQueryTokens.includes(rawTitle)) score += 80;
