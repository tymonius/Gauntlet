...
- Actual faction resources carry one compact **STANDARD [RESOURCE] CAP** line sourced from the authoritative tracked-value maximum. An uncapped resource such as Intel reads **STANDARD INTEL CAP · NONE**. The derived Financier Capital Limit is not itself a resource, so its equivalent line reads **RULES MAXIMUM · NONE**. Operation Progress is a progression value rather than a resource and therefore carries no cap text, but it retains the same horizontal divider below the title block.
...
| Capital Limit | 0–15 | selected Financier Leader Card | Capital Limit has no rules maximum. The printed tracker exposes only 1–15 because 0 is represented by full cover. Fifteen is a practical normal-play range with headroom, rather than a theoretical maximum, and keeps the registration bands large enough to use comfortably. |
...
When a resource has a hard rules maximum, the tracker should at minimum cover that value and may include modest additional headroom where near-term effects are expected to raise the cap. When a resource is uncapped or a tracked game-state value has no fixed rules maximum, choose a practical capacity from normal-game behavior and add a small buffer rather than attempting an unusably dense theoretical maximum.
