...
function validateV06EndpointMovement(game: GameState, action: GameAction): void {
  ...
  if (isOpponentBeyondGauntletSpace(destination, action.playerId)) {
    if (!destination.occupant || destination.occupant === action.playerId) throw new GameActionError('A player advances beyond the Gauntlet only to force the opponent to make a Last Stand.');
    if (origin.kind !== 'territory' || origin.controller !== action.playerId) throw new GameActionError('The final Territory must be captured before the opponent can be forced to make a Last Stand.');
  }
}
...
function activeBattleSnapshot(battle: BattleState): BattleState {
  ...
  for (const side of [active.attacker, active.defender]) {
    if (side.handCommit?.canceled) side.handCommit = undefined;
    side.battleDrawPlayed = side.battleDrawPlayed.filter((card) => !card.canceled);
  }
  return active;
}
...
function correctAddedCardDestinations(game: GameState, battle: BattleState): void { for (const side of [battle.attacker, battle.defender]) { const player = game.players[side.playerId]; for (const card of side.battleDrawPlayed.filter((played) => played.origin === 'hand')) { if (card.cleanupDestination === 'hand') continue; const index = player.zones.discard.indexOf(card.cardId); if (index >= 0) player.zones.discard.splice(index, 1); if (!player.zones.graveyard.includes(card.cardId)) player.zones.graveyard.push(card.cardId); } } }
...
function appendLastStandVictoryLog(game: GameState, winner: PlayerID, defeatedPlayer: PlayerID): void { game.log.push({ id: `${game.id}-event-${game.log.length + 1}`, turn: game.turn, actor: winner, type: 'last_stand_won', message: `${game.players[winner].name} defeated ${game.players[defeatedPlayer].name} in a Last Stand battle and ran the Gauntlet.`, payload: { winner, defeatedPlayer }, visibility: 'public' } satisfies GameEvent); }
function finalizeLastStandResolution(result: ApplyGameActionResult, attacker?: PlayerID, defender?: PlayerID): void { if (result.state.battle || !attacker || !defender || lastResolvedBattleWinner(result.state) !== attacker) return; result.state.winner = attacker; result.state.phase = 'game_over'; result.state.priorityPlayer = attacker; result.state.pendingMilitaryChoice = undefined; result.state.militaryChoiceQueue = undefined; result.state.pendingMilitaryTimingChoice = undefined; result.state.militaryTimingChoiceQueue = undefined; result.state.pendingDiplomatChoice = undefined; result.state.pendingFinancierChoice = undefined; result.state.financierChoiceQueue = undefined; result.state.pendingLeaderAbilityWindow = undefined; result.state.pendingAssetBankDiscards = undefined; appendLastStandVictoryLog(result.state, attacker, defender); }
...
export function applyGameAction(game: GameState, action: StateAction): ApplyGameActionResult {
  ...
  if (action.type === 'resolve_battle' && !result.state.battle) recordBattleAftermath(result, battleBeforeResolution);
  ...
  if (action.type === 'resolve_battle' && !result.state.battle) resetLeaderAbilityUsageAfterBattle(result.state);
