...
describe('public-media TTS proxy', () => {
  ...
  it('proxies a clean asset URL to the exact approved immutable release asset', async () => {
    ...
    const response = await worker.fetch(
      new Request(PUBLIC + 'Gauntlet_v0.7.0_TTS_Environment_Table.png'),
      {},
    );
    ...
    expect(response.headers.get('x-gauntlet-tts-source')).toBe('pr-909-fe5940376a74');
  });
  ...
  it('rewrites only the approved release prefix in the public mod JSON', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      APPROVED + 'Gauntlet_v0.7.0_TTS_PR909_Preview.json',
      { redirect: 'follow' },
    );
    expect(text).not.toContain(APPROVED);
    expect(text).toContain(PUBLIC + 'Gauntlet_v0.7.0_TTS_Environment_Table.png');
    expect(text).toContain(PUBLIC + 'Gauntlet_v0.7.0_TTS_Cards_neutral_01.png');
  ...
  it('does not proxy arbitrary nested paths', async () => {
    ...
    const response = await worker.fetch(
      new Request(PUBLIC + 'nested/file.png'),
      {},
    );
