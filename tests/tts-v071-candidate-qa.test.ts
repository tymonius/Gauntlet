...
describe('v0.7.1 release / TTS QA boundary', () => {
  ...
  it('records the stable v0.7.1 manual QA as passed and explicitly Workshop-approved', () => {
    ...
    expect(stableQa.notes.some((note: string) => /good for rollout/i.test(note))).toBe(true);
  });
});
