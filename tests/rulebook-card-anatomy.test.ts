...
const anatomySource = read('rulebook/player-facing/card-anatomy.md');
const styles = read('rulebook/card-anatomy.css');

describe('Browser Rulebook card anatomy guide', () => {
  it('loads the browser enhancement assets while keeping the content in player-facing Markdown', () => {
    expect(index).toContain('href="card-anatomy.css"');
    expect(index).toContain('src="card-anatomy.js"');
    expect(anatomySource).toContain('## Card anatomy');
    expect(anatomySource).toContain('### Arcane trait mark');
  });
  ...
  it('authors all current standard playable-card elements in the source itself', () => {
    for (const label of [
      'Card name',
      'Card value',
      'Faction identity',
      ...
      expect(anatomySource).toContain(label);
    }
    expect(anatomySource).not.toContain('Reminder');
    expect(anatomySource).not.toContain('faction-specific procedure');
  ...
  it('shows the Arcane trait on a real card with only the cropped bottom edge feathered', () => {
    ...
    expect(anatomySource).toContain("its color follows the card's faction identity");
