...
> **Scaffold only.** This will be the single complete technical rules corpus for Gauntlet, including every faction. The `RULES-COVER` markers below bind each part to semantic rule domains in `config/rules-surface-contract.json`. Exact mechanical text will be projected or assembled from authority rather than independently re-authored here.
