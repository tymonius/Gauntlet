# Gauntlet v0.6 Military Faction Guide

> **Definitive v0.6 Military faction source.** This guide governs all Military-specific rules, Leaders, Orders, supplemental components, and the canonical twelve-card Military pool. General movement, battle, capture, Asset, Overlay, and victory rules remain in the core rulebook.

# 1. Military overview

The Military faction converts battlefield victories into Command, movement, retreat pressure, and control. Military has no additional victory condition; it wins by running the Gauntlet.

## At a glance

| **ELEMENT** | **MILITARY RULE** |
|---|---|
| **Victory** | Run the Gauntlet. |
| **Resource** | Command, maximum 2. |
| **Resource gain** | The first time each turn you win a battle, gain 1 Command, up to the maximum. |
| **Leaders** | General and Commandant. |
| **Faction pool** | 12 Military card titles. |
| **Unique card** | Shock and Awe, cost 5; maximum one copy per Playable Deck. |

# 2. Components and setup

## Components

A Military Deck includes:

- one Military Leader Card: **General** or **Commandant**;
- one shared **Military Command Tracker**; and
- any Military cards included in the Playable Deck.

## Setup

1. Place the chosen Leader Card face up near your play area.
2. Place the Military Command Tracker directly beneath it, fully covered, to show 0 Command.
3. Slide the Leader Card upward to the 1 or 2 registration line as Command changes.

The Leader Card is the Command indicator; no separate Command token is used.

# 3. Command and Orders

## Command

Military may have up to 2 Command.

The first time each turn you win a battle, gain 1 Command, up to the maximum. This trigger is not limited to your own turn. A defensive victory during the opponent's turn can generate Command, but only the first Military victory during that turn does so.

Command gained above the maximum is lost. Winning while already at 2 Command still counts as the first Military victory of that turn.

## Orders

Spend Command to use Orders printed on your Leader Card.

- Use an Order only at the timing printed in that Order.
- Spend the listed Command when the Order is used unless an effect reduces or replaces that cost.
- If you do not have enough Command, you cannot use the Order.
- An Order changes only the movement, battle, retreat, or capture specified by its text.

# 4. Military Leaders

## General

<img src="../../../../images/sketches/general.png" alt="General" />

**Archetype:** Attack, forward pressure, and tempo  
**Motto:** *Forward. Again.*

The General leads from the decisive point, using Command to advance, reinforce an attack, and press a defeated opponent before they can recover.

### General Orders

| **ORDER** | **COST** | **EFFECT** |
|---|---:|---|
| **Onward** | 1 Command | Move one additional position this turn. Resolve this movement one position at a time. It may initiate a battle. |
| **Rally** | 1 Command | Before dice are rolled in a battle you initiated, add +1 to your battle total. |
| **Rout** | 2 Command | After you win a battle you initiated, move one position toward the opponent's end of the Gauntlet. This movement may initiate another battle. |

## Commandant

<img src="../../../../images/sketches/commandant.png" alt="Commandant" />

**Archetype:** Defense, counterattack, and control  
**Motto:** *We hold. They break.*

The Commandant turns a defended position into lasting control, using Command to strengthen a defense, drive an attacker farther back, or capture occupied ground immediately.

### Commandant Orders

| **ORDER** | **COST** | **EFFECT** |
|---|---:|---|
| **Entrench** | 1 Command | Before dice are rolled in a battle you did not initiate, add +1 to your battle total. |
| **Repel** | 1 Command | After you win a battle you did not initiate, the defeated opponent retreats one additional position, if able. |
| **Fortify** | 2 Command | After you win a battle while occupying an enemy-controlled Territory, capture that Territory immediately. |

# 5. Military-specific rules

## Card sources during battle

- A card committed from your hand normally goes to your Graveyard during battle cleanup.
- A card played from your Battle Hand normally goes to your Discard Pile during battle cleanup.
- A card added to a battle after the initial reveal retains its source and follows the normal destination for that source unless the effect specifies otherwise.

## Initiating battles

You initiated a battle if your movement or effect caused it to begin. If the opponent's movement or effect caused it to begin, you did not initiate it.

A follow-up battle is a new battle. Resolve a new commitment step, form new Battle Hands, and apply all once-per-battle effects separately.

Under the core movement rules, beginning a battle ends the current movement sequence and any unused movement is lost. **Onward** therefore extends movement only until a battle begins. **Rout** and other effects that explicitly permit movement after a battle create a new movement sequence.

## Territory terms

- **Occupying an enemy-controlled Territory:** Your Player Token is on a Territory controlled by the opponent after you won there but before you captured it.
- **Defending a Territory you control:** Your Player Token occupied a Territory you control when the battle began.
- **Enemy-controlled Territory:** A Territory currently controlled by the opponent, whether or not you occupy it.

Effects that require a Territory do not apply during a Last Stand beyond the Gauntlet.

## Encampment

Encampment is an Overlay and does not use Asset capacity.

- Place it on a Territory as directed by its text.
- It faces the same direction as the Territory beneath it unless an effect states otherwise.
- Leaving the Territory or temporarily losing occupation does not remove it.
- When an opponent gains control of that Territory, place Encampment in its owner's Graveyard.

## Additional cards after reveal

When a Military effect allows a card to be added after the initial battle reveal, reveal that card face up. Its Battle effect must still be able to resolve at that timing. The card retains its source and follows the destination stated by the effect or the normal destination for that source.

## Conflicting victory benefits

Several Military effects change the consequences of winning a battle. Follow the specific effect.

War Crimes and Shock and Awe prohibit the listed movement, capture, or Orders resulting from that victory after their special effect is chosen. If multiple effects would replace the same movement, retreat, capture, or card destination, apply only combinations that can all legally resolve.

# 6. Canonical Military card pool

The following entries are the canonical v0.6 Military names, costs, uniqueness, and exact player-facing text.

## Unbroken Ranks

**Cost:** 1

> **Action:** Bank this as an Asset. After you win a battle in which you used no Orders, you may discard it to gain 1 Command.
>
> **Battle:** If you win this battle and used no Orders in it, gain 1 Command.

## Battlefield Promotion

**Cost:** 2

> **Action:** During an Action Opportunity after you win a battle this turn, return one card you played from your Battle Hand in that battle from your Discard Pile to your hand.
>
> **Battle:** If you win, return one other card you played from your Battle Hand to your hand instead of discarding it during cleanup.

## Encampment

**Cost:** 2  
**Card form:** Territory Overlay

> **Action:** Place this as an Overlay on a Territory you occupy and control.
>
> **Overlay:** At the end of your turn, if you occupy and control this Territory, gain 1 Command. When an opponent gains control of it, put this in its owner's Graveyard.
>
> **Battle:** If you win while defending a Territory you control, place this on it as an Overlay during cleanup.

## Rearguard

**Cost:** 2

> **Action:** Bank this as an Asset. After you lose a battle and retreat, when your opponent would enter your position that turn using an Order or card effect, you may discard this to prevent that movement. No Command is spent; any card used returns to its owner's hand.
>
> **Battle:** If you lose and retreat, bank this during cleanup.

## Brothers in Arms

**Cost:** 2

> **Action:** Bank this as an Asset. Before cards are committed from hand in a battle involving you, you may discard this to delay your hand commitment until after both players form their Battle Hands. Then either commit one card from your hand and choose one card from your Battle Hand, revealing both face up together, or choose neither. Both Battle effects must still be able to resolve.
>
> **Battle:** If this is in your initial Battle Hand and you committed no card from your hand, you may choose this and also commit one card from your hand face up. Both Battle effects must still be able to resolve.

## Field Command

**Cost:** 3

> **Action:** Bank this as an Asset. After using a 1-Command Order, you may discard this to use your Leader's other 1-Command Order once this turn, at its next legal timing, for 0 Command.
>
> **Battle:** After using a 1-Command Order during this battle, you may use your Leader's other 1-Command Order once this turn, at its next legal timing, for 0 Command. If you do, put this in your Graveyard after that Order resolves.

## Reserve Force

**Cost:** 3

> **Action:** Bank this as an Asset with another card from your hand that has a Battle effect face down beneath it. After all cards in a battle involving you are revealed, you may discard this to reveal the stored card face up for its Battle effect, if that effect can still resolve. Put the stored card in your Graveyard during cleanup, or immediately if this leaves play before deployment.
>
> **Battle:** After all cards in the battle are revealed, you may replace this with a card from your hand whose Battle effect can still resolve. If you do, put this in your Graveyard, reveal that card face up for its Battle effect, and put that card in your Graveyard during cleanup. Otherwise, discard this during cleanup.

## Give Chase

**Cost:** 3

> **Action:** During an Action Opportunity after you win a battle you initiated this turn, move one position toward the opponent's end of the Gauntlet. This may initiate a battle.
>
> **Battle:** If you win and initiated this battle, after the opponent retreats, move one position toward their end of the Gauntlet. This may initiate a battle.
>
> **Pursuit:** In a battle initiated this way, you cannot commit a card from your hand or use Orders. When forming your Battle Hand, draw one fewer card for each battle already fought this turn beyond the first, to a minimum of zero. After the movement, put this in your Graveyard.

## Hold the Line

**Cost:** 4

> **Action:** Bank this as an Asset. When a battle begins in which you defend a Territory you control, before cards are committed from hand, you may put this in your Graveyard to use the effect below.
>
> **Battle:** If you are defending a Territory you control, use the effect below, then put this in your Graveyard during cleanup.
>
> **Effect:** After all cards in the battle are revealed, draw two additional cards into your Battle Hand and immediately reveal up to one of them face up for its Battle effect, if that effect can still resolve. If you lose, after you retreat, the opponent captures that Territory immediately.

## Countercharge

**Cost:** 4

> **Action:** Bank this as an Asset. After you win a battle you did not initiate, you may put this in your Graveyard to use the effect below.
>
> **Battle:** If you win this battle and did not initiate it, use the effect below, then put this in your Graveyard.
>
> **Effect:** After the opponent retreats, move one position toward their end of the Gauntlet. This may initiate a battle.

## War Crimes

**Cost:** 4

> **Action:** Bank this as an Asset. After you win a battle, you may put this in your Graveyard to use the effect below.
>
> **Battle:** If you win, you may use the effect below. If you do, put this in your Graveyard during cleanup.
>
> **Effect:** Put every card your opponent played from their Battle Hand in that battle in their Graveyard instead of their Discard Pile, and make them retreat one additional position. You cannot move, capture a Territory, or use an Order as a result of that victory.

## Shock and Awe

**Cost:** 5  
**Unique:** Maximum one copy per Playable Deck

> **Action:** Bank this as an Asset. When you initiate a battle on an enemy-controlled Territory, before cards are committed from hand, you may put this in your Graveyard to use the effect below.
>
> **Battle:** If you are attacking on an enemy-controlled Territory, use the effect below, then put this in your Graveyard during cleanup.
>
> **Effect:** After all cards in the battle are revealed, you may reveal one additional card from your hand face up for its Battle effect, if that effect can still resolve. Put that card in your Graveyard during cleanup. If you lose, retreat one additional position after your normal retreat. If you win, choose **Breakthrough** or **Consolidate**.
>
> **Breakthrough:** Choose only if the opponent can retreat one additional position. They do so after their normal retreat; then move one position toward their end of the Gauntlet. This movement cannot initiate a battle.
>
> **Consolidate:** Capture the contested Territory immediately, then set your Command to 2.
>
> **Limit:** After either option, you cannot move again, capture another Territory, or use an Order as a result of that victory.

# 7. Card-pool summary

| **COST** | **MILITARY CARDS** |
|---:|---|
| **1** | Unbroken Ranks |
| **2** | Battlefield Promotion, Encampment, Rearguard, Brothers in Arms |
| **3** | Field Command, Reserve Force, Give Chase |
| **4** | Hold the Line, Countercharge, War Crimes |
| **5** | Shock and Awe — Unique |

# Appendix A. Military quick reference

## Command

Maximum 2. The first time each turn you win a battle, gain 1 Command. Spend Command to use Orders. Track 0, 1, or 2 by sliding the Leader Card over the Command Tracker.

## General Orders

| **ORDER** | **COST** | **EFFECT** |
|---|---:|---|
| **Onward** | 1 | Move one additional position this turn; may initiate a battle. |
| **Rally** | 1 | Before dice in a battle you initiated, add +1 to your battle total. |
| **Rout** | 2 | After winning a battle you initiated, move one position toward the opponent's end of the Gauntlet; may initiate another battle. |

## Commandant Orders

| **ORDER** | **COST** | **EFFECT** |
|---|---:|---|
| **Entrench** | 1 | Before dice in a battle you did not initiate, add +1 to your battle total. |
| **Repel** | 1 | After winning a battle you did not initiate, the defeated opponent retreats one additional position, if able. |
| **Fortify** | 2 | After winning while occupying an enemy-controlled Territory, capture it immediately. |

## Reminders

- Only the first Military victory during each turn generates Command.
- Cards committed from hand normally go to the Graveyard.
- Cards played from Battle Hands normally go to the Discard Pile.
- Territory-only effects do not apply during a Last Stand beyond the Gauntlet.
- Obey the movement, capture, and Order limits printed on War Crimes and Shock and Awe.

---

Gauntlet v0.6 © 2026 Tymon Scott. All rights reserved. Military Faction Guide.
