...
ROOT = Path('/mnt/data/gauntlet_v0_5_3')
VERSION = 'v0.5.3'
DATE = 'June 20, 2026'
...
def export_json() -> Path:
    ...
    path = ROOT / 'Gauntlet_v0.5.3_Canonical_Data.json'
...
def export_deck_lists_md() -> Path:
    ...
    for name, deck in DECKS.items():
        ...
        lines.append('')
    path = ROOT / 'Gauntlet_v0.5.3_Playtest_Deck_Lists.md'
...
def build_docx() -> Path:
    ...
    doc.add_paragraph('\n')
    add_rule_box(doc, 'Playtest status', 'This is a pre-release ruleset. Card costs and individual effects remain provisional, but all games recorded as v0.5.3 should use the rules and card text in this document without substitution.')
    doc.add_page_break()
    ...
    contents = [
        ...
        'Appendix B. v0.5.3 change summary',
    ]
    ...
    add_rule_box(doc, 'Current release', f'{VERSION} - Attrition Patch. Released for physical playtesting on {DATE}.')
    ...
    add_rule_box(doc, 'Objective', 'Advance through the opposing Territories and enter the enemy Heartland. If the opposing token occupies the Heartland, defeat it there to win.')
    ...
    key_terms = [
        ...
        ('HOMELAND ADVANTAGE','A defender on a Territory they control wins tied battle totals.'),
    ...
    for x in [
        ...
        'A defender on a Territory they control wins ties. All other ties are rerolled as part of the same battle; ongoing Battle effects remain active.',
    ]: add_bullet(doc,x)
    ...
    for x in [
        ...
        'Assets, Conditions, Overlays, the new Territory, and Homeland Advantage are checked again.',
    ...
    # Appendix changes
    doc.add_heading('Appendix B. v0.5.3 change summary', level=1)
    doc.add_heading('Changes from v0.5.1 and earlier', level=2)
    add_bullet(doc, 'Battle-card destinations now depend on source: hand-origin cards go to the Graveyard; battle-drawn cards recycle to the discard pile.')
    add_bullet(doc, 'The incomplete-draw rule now explicitly instructs players to draw as many cards as possible.')
    add_bullet(doc, 'Attrition now converts an opponent\'s recyclable battle-draw cards into permanent losses after that opponent loses.')
    add_bullet(doc, 'Heartland replaces Home as the name of each player\'s endpoint and victory space.')
    add_bullet(doc, 'Card wording that formerly assumed every played Battle card entered the Graveyard now uses source-specific or normal-destination language.')
    ...
    path = ROOT / 'Gauntlet_v0.5.3_Complete_Playtest_Guide.docx'
    doc.save(path)
    return path
...
def draw_heartland_card(c:canvas.Canvas,x:float,y:float,label:str='HEARTLAND',deck_name:str|None=None) -> None:
    ...
    fit_paragraph(c,'<b>Your force begins here.</b><br/><br/>An opponent wins by entering this Heartland while it is empty, or by entering and defeating your token here.',x+0.25*inch,y+0.65*inch,CARD_W-0.5*inch,CARD_H-1.65*inch,10,7.5,1.25)
...
def build_four_deck_pdf() -> Path:
    ...
    for deck_name,deck in DECKS.items():
        ...
        items.append({'kind':'aid','deck':deck_name,'title':'BATTLE & DESTINATIONS','lines':[
            'Optional one card from hand.','Draw three; select up to one.','Reveal; cancel; resolve effects.','Roll and determine winner.','Loser retreats; attacker may occupy.','Hand card to Graveyard; battle-draw card to discard.'
        ]})
    # 36 cards/deck = 144 cards = 16 sheets
    assert len(items)==144, len(items)
    path=ROOT/'Gauntlet_v0.5.3_Four_Deck_Playtest_Set.pdf'
    draw_card_pages(path,items,'Gauntlet v0.5.3 Four-Deck Playtest Set')
    return path
...
def build_master_pool_pdf() -> Path:
    ...
    # 81 exactly (54+25+2)
    assert len(items)==81
    path=ROOT/'Gauntlet_v0.5.3_Master_Card_Pool.pdf'
    draw_card_pages(path,items,'Gauntlet v0.5.3 Master Card Pool')
    return path


def build_playtest_forms_pdf() -> Path:
    path=ROOT/'Gauntlet_v0.5.3_Playtest_Forms.pdf'
    c=canvas.Canvas(str(path),pagesize=letter)
    for page in range(4):
        margin=0.55*inch
...
- `Gauntlet_v0.5.3_Complete_Playtest_Guide.docx` - editable canonical guide.
- `Gauntlet_v0.5.3_Complete_Playtest_Guide.pdf` - print-ready guide.
- `Gauntlet_v0.5.3_Four_Deck_Playtest_Set.pdf` - four complete 30-card decks, twelve Territories, four Heartlands, and eight player aids.
- `Gauntlet_v0.5.3_Master_Card_Pool.pdf` - one copy of all 54 main-deck cards, all 25 Territories, and two Heartlands.
- `Gauntlet_v0.5.3_Playtest_Forms.pdf` - four game record sheets.
- `Gauntlet_v0.5.3_Canonical_Data.json` - editable structured source data.
- `Gauntlet_v0.5.3_Playtest_Deck_Lists.md` - deck manifests.
...
The Four-Deck Playtest Set is organized deck-by-deck and labels every card at the bottom for sorting. The Shadow Directorate is the v0.5.3 Attrition trial list; Attrition replaces Court Martial at equal cost.
...
def make_zip(paths: List[Path]) -> Path:
    zpath=ROOT/'Gauntlet_v0.5.3_Print_and_Play_Package.zip'
